...
pub async fn list(query: Query<ListArticleRequestQuery>) -> impl Responder {
    use toy_blog_endpoint_model::ListArticleSortPolicy as SortPolicy;

    let x: ComposeInternalError<ListArticleResult> = match GLOBAL_FILE.parse_file_as_json() {
        Ok(entries) => {
            let mut json: ListArticleResponse = entries.into();
    ...
    match x {
        Ok(list) => EndpointRepresentationCompiler::from_value(list.unwrap())
            .into_json()
            .map_body(|_, y|
                BoxBody::new(serde_json::to_string(&y).expect("map HTTP"))
            ),
        Err(err) => EndpointRepresentationCompiler::from_value(err).into_plain_text()
            .map_body(|_, y| BoxBody::new(y))
    }
}
